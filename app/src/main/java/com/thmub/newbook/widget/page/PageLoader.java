...
public abstract class PageLoader {
    ...
    /*****************************Initialization*******************************/
    PageLoader(PageView pageView, ShelfBookBean bookShelfBean) {
        this.mPageView = pageView;
        this.bookShelfBean = bookShelfBean;
        this.mContext = pageView.getContext();
        this.mCurChapterPos = bookShelfBean.getCurChapter();
        this.mCurPagePos = bookShelfBean.getCurChapterPage();
...
//            mPageChangeListener.onCategoryFinish(book.getBookChapterList());
     ...
     * 加载错误
     */
    void showChapterError(String msg) {
        if (mCurChapter == null) {
     ...
     * 翻页完成
     */
    void finishPaging(PageAnimation.Direction direction) {
        if (!isChapterListPrepare) {
            return;
        }
        switch (direction) {
            case NEXT:
                ...
                } else if (mCurChapterPos < bookShelfBean.getBookChapterList().size() - 1) {
                    mCurChapterPos = mCurChapterPos + 1;
                    mCurPagePos = 0;
                    mPreChapter = mCurChapter;
                    mCurChapter = mNextChapter;
        ...
        bookShelfBean.setCurChapter(mCurChapterPos);
        bookShelfBean.setCurChapterPage(mCurPagePos);
    ...
    private synchronized void drawBackground(final Canvas canvas, TxtChapter txtChapter, TxtPage txtPage) {
        ...
        if (!bookShelfBean.getBookChapterList().isEmpty()) {
            ...
            String progress = (txtChapter.getStatus() != TxtChapter.Status.FINISH) ? ""
                    : BookShelfUtils.getReadProgress(mCurChapterPos, bookShelfBean.getBookChapterListSize(), mCurPagePos, mCurChapter.getPageSize());
    ...
    private void switchToPageOffset(int offset) {
        switch (offset) {
            case 1:
                if (mCurPagePos < mCurChapter.getPageSize() - 1) {
                    mCurPagePos = mCurPagePos + 1;
                } else if (mCurChapterPos < bookShelfBean.getBookChapterListSize() - 1) {
                    mCurChapterPos = mCurChapterPos + 1;
                    mPreChapter = mCurChapter;
                    mCurChapter = mNextChapter;
                    mNextChapter = null;
...
//                mTextPaint.setColor(isLight ? ThemeStore.accentColor(mContext) : readSettingManager.getTextColor());
            ...
            if (pagePos == chapter.getPageSize() - 1) {
                String sign = "\u23af \u23af";
                if (chapterPos == bookShelfBean.getBookChapterListSize() - 1) {
...
//                mTextPaint.setColor(isLight ? ThemeStore.accentColor(mContext) : readSettingManager.getTextColor());
    ...
    boolean hasNext(int pageOnCur) {
        ...
        return mCurChapterPos + 1 < bookShelfBean.getBookChapterListSize();
    }
    ...
    void parseNextChapter() {
        ...
        if (nextChapterPos >= bookShelfBean.getBookChapterListSize()) {
            mNextChapter = null;
            return;
        }
